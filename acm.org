...
public:
    int findUnsortedSubarray(vector<int>& nums) {
      ...
      int min = nums[0], ni = -1;
      for(int i = 1; i < n; i++){
        if(ni == -1 && nums[i] < nums[i-1]){
	  min = nums[i], ni = i;
	}else if(ni != -1){//find min before
	  if(nums[i] < min) min = nums[i], ni = i;
	}
      }
      ...
      for(int i = n-1-1; i>=0; i--){
        if(mi == -1 && nums[i] > nums[i+1]){
	  max = nums[i], mi = i;
	}else if(mi != -1){//find max before
	  if(nums[i] > max) max = nums[i], mi = i;
	}
      }
