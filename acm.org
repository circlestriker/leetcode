...
public:
    ...
    TreeNode* deserialize(string data) {
       ...
       TreeNode * h = NULL;
       queue<TreeNode*> q;
       h->val = data[0], h->left = NULL, h->right = NULL;
       q.push(h);
	 ...
	 if(c == 'N') r->left = NULL;
         ...
         p->val = data[i], p->left = NULL, p->right = NULL;
	 r->left = p;
	 q.push(p);
	 }
	 if(i+2 < data.size()){
	   TreeNode * p2 = new TreeNode;
	   p2->val = data[i+2], p2->left = NULL, p2->right = NULL;
	   r->right = p2;
	   q.push(p2);
	 }
	 q.pop();
