https://www.onlinegdb.com/fork/4Dsy5CprC
...
//two array as a whole, then find the kTH value
double kthNum(int * p1, int start1, int end1, int * p2, int start2, int end2, int k){
  ...
  if(half1 < half2){
    return kthNum(p1, start1+half, end1, p2, start2, end2, k-half); //throw away the smaller half(k/2 values) each time.
  }else{
